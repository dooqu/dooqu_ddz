...
#define write_to_everyone(___desk, format, ...) \
        for(int ___desk_pos_index = 0; ___desk_pos_index < ddz_desk::DESK_CAPACITY; ++___desk_pos_index)\
        {\
            ws_client* __pos_ws_client__ = ___desk->pos_client(___desk_pos_index);\
            if(__pos_ws_client__ != NULL)\
            {\
                __pos_ws_client__->write_frame(true, ws_framedata::opcode::TEXT, format, ##__VA_ARGS__);\
            }\                
        }\
...
protected:
    ...
    //临时变量
    ws_client* temp_landlord;


    //玩家数组
    ws_client* clients_[DESK_CAPACITY];
...
public:
    ...
    ws_client* get_landlord();


    void set_landlord(ws_client*);
    ...
    char* id()
    {
        return &this->desk_id_[0];
    }

    void set_id(int i)
    {
        int n = sprintf(this->desk_id_, "%d", i);
        this->desk_id_[n] = 0;
    }
    ...
    //设置制定位置索引的玩家信息
    void set(int pos_index, ws_client* client);
    ...
    int bid_pos()
    {
        return this->bid_pos_;
    }

    //该桌子是否已经开始叫牌
    bool is_status_bided()
    {
        return this->is_bided_;
    }

    //设置该桌的叫牌状态
    void set_status_bided(bool is_bided)
    {
        this->is_bided_ = is_bided;
    }


    //将焦点移动到下一个轮值叫牌玩家的位置上，并返回其索引位置
    int move_to_next_bid_pos()
    {
        this->bid_pos_ = ++this->bid_pos_ % DESK_CAPACITY;
        return this->bid_pos_;
    }
    ...
    //获取指定玩家在桌子上的位置索引
    int pos_index_of(ws_client* client);
    ...
    //获取制定索引位置的玩家信息
    ws_client* pos_client(int pos_index);
...
private:
    ...
    int desk_count_;
public:
    ddz_desk_list() : desks_(NULL), desk_count_(0)
    {
    }

    void init(int desk_count)
    {
        assert(desk_count >= 0);

        if(this->desk_count_ != desk_count)
        {
            if(this->desks_ != NULL)
            {
                delete [] this->desks_;
            ...
            desks_ = new ddz_desk[desk_count];
    ...
    {
        assert(index >= 0 && index < this->desk_count_);

        if (index < 0 || index >= desk_count_)
            return NULL;
